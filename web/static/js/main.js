...
async function showMovieDetails(movieId, isTMDB = false) {
    try {
        ...
        const endpoint = isTMDB ? `/api/tmdb/movie/${movieId}` : `/api/movie/${movieId}`;
        ...
        if (isTMDB) {
            renderTMDbMovieDetails(movie, overlay, movieId);
        } else {
...
function displayTrendingResults(movies) {
    ...
    trendingResults.innerHTML = movies.map(movie => `
        <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all cursor-pointer group" data-movieid="${movie.id}">
            <div class="h-40 bg-gray-200 overflow-hidden relative">
                ${movie.poster_path ? 
                    `<img src="https://image.tmdb.org/t/p/w500${movie.poster_path}" alt="${movie.title}" class="w-full h-full object-cover group-hover:opacity-90 transition-opacity">` : 
                    `<div class="w-full h-full flex items-center justify-center text-gray-500">No image</div>`}
                <div class="absolute bottom-2 left-2 bg-yellow-400 text-black text-xs font-bold px-2 py-1 rounded flex items-center">
                    ★ ${movie.vote_average.toFixed(1)}
                </div>
            </div>
            <div class="p-3">
                <h3 class="font-bold text-sm mb-1 truncate">${movie.title}</h3>
                <p class="text-xs text-gray-600">${movie.release_date.split('-')[0]}</p>
            </div>
        </div>
    `).join('');
    ...
    document.querySelectorAll('#trendingResults [data-movieid]').forEach(card => {
        card.addEventListener('click', () => {
            ...
            showMovieDetails(movieId, true); // true indicates it's from TMDB
        });
    });
}

function renderTMDbMovieDetails(movie, overlay, movieId) {
    const content = overlay.querySelector('#movieDetailsContent');
